  ...
  return (
    <div className="group flex flex-col min-w-0">
      <Link
        ...
        className="relative block overflow-hidden rounded-xl sm:rounded-2xl bg-sand-50 aspect-[4/5] card-elevated"
      >
        <Image
        ...
        {/* Badge */}
        {discount ? (
          <span className="absolute left-2 top-2 sm:left-3 sm:top-3 rounded-full bg-clay px-2 py-0.5 text-[9px] sm:text-xs font-bold text-white shadow-sm">
            -{discount}%
          </span>
        ) : (
          <span className="absolute left-2 top-2 sm:left-3 sm:top-3 rounded-full bg-sand-100/95 backdrop-blur-sm px-2 py-0.5 text-[9px] sm:text-xs font-semibold text-charcoal shadow-sm">
            Koleksi
          </span>
        )}
        <div className="absolute inset-x-0 bottom-0 translate-y-full bg-gradient-to-t from-black/50 via-black/20 to-transparent p-2.5 sm:p-3 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100 hidden sm:block">
