...
// Drawer tidak dipakai di identitas Easthtic (hanya stub agar build pass jika file masih ada).
// Untuk situs identitas, Header tidak memanggil CartDrawer — file ini dibiarkan kosong-pass.
...
export function CartDrawer() {
  ...
  return (
    <>
      ...
      />
      <div
        className={`fixed right-0 top-0 z-50 flex h-full w-full max-w-[420px] flex-col bg-cream shadow-2xl transition-transform duration-300 ${isDrawerOpen ? "translate-x-0" : "translate-x-full"}`}
        role="dialog"
        aria-modal="true"
                ...
                {lines.map((line: CartLine) => (
                  <li key={line.id} className="flex gap-4">
                    ...
                    <div className="flex flex-1 flex-col">
                      ...
                      <p className="mt-0.5 text-xs text-stone-500">{line.merchandise.selectedOptions.map((o: { value: string }) => o.value).join(" • ")}</p>
