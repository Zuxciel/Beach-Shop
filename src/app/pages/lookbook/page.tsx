      ...
      <div className="mt-8 flex justify-center gap-2 overflow-x-auto scrollbar-hide">
        {lookbookItems.map((item) => (
          <a
            key={item.id}
            href={`#${item.id}`}
            className="whitespace-nowrap rounded-full border border-sand-200 bg-white px-4 py-2 text-xs font-medium hover:border-ocean hover:text-ocean transition shadow-xs"
          >
            {item.title}
          </a>
        ))}
